    ...
    $.post(base_url + "/home/get_all_products/" + index, {  ajax: true, brand: brand, catgery : catgery }, function(data) {
        ...
        if(data.status){
            ...
            for(var i = 0; i< data1.length; i++){
                ...
                else {var per = (100 * bal)/data1[i].actual_cost;}
                div +=' <div class="blk tp_mar20">'+
                    '<div class="deal"> <img src="'+base_url+'assets/admin/products/'+data1[i].image+'" class="indxprct">'+

                    '<a href="'+base_url+'home/product_details/'+data1[i].id+'">'+
                   
                    '</a>'+
                    '<div class="su_box100 dealbg1">'+
                    '<h4><img src="'+base_url+'assets/admin/brand/'+data1[i].partner_img+'" class="logoleft">Deals with '+data1[i].partner+'</h4>'+
                    '</div>'+
                    ...
                    '<div class="su3bx">'+
                    '<div class="offferrate3">'+Math.round(per)+'% Off </div>'+
                    '</div>'+
