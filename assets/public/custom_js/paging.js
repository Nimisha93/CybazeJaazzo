$(document).ready(function(){
    ...
    //Transaction Details - TL / CA
    function load_demo2(index) {
        index = index || 0;
        var search = $('#search2').val();

        $.post(base_url + "view_cm_transaction/" + index, { ajax: true,search:search}, function(data) {
            $('#tbody2').html("");
            $('.body_blur').hide();
            ...
            if(data.data.length>0){
                ...
                for(var i = 0; i< data1.length; i++){
                   ...
                   // var dt =date_format(data1[i].transaction_date,"d/m/Y H:i:s");
                    tr += '<tr>'+
                        '<td>'+sl_no+'</td>'+
