     ...
     function get_cpWallet($id){
        ...
        $query="SELECT bn.id,bn.login_id,nc.name,truncate(bn.wallet_total,2) as wallet_total,truncate(bn.bill_total,2) as bill_total,bn.purchased_on as purchase_date, DATE_FORMAT(bn.purchased_on,'%d-%b-%Y')as purchsed_on,bn.channel_partner_id
         ,truncate(bn.total_commission,2) as total_commission FROM gp_purchase_bill_notification bn LEFT JOIN gp_login_table log ON log.id=bn.login_id 
            LEFT JOIN gp_normal_customer nc ON log.user_id=nc.id WHERE bn.channel_partner_id = '$id' and bn.status = '1'";
    ...
    function updatewallet($noty_id, $total_discount, $total_amount){
        ...
        if($qry->num_rows()>0)
        {
          ...
          //  echo $this->db->last_query();exit();
            ...
            $entry_id1 = add_acc_entry($noty_id,$total_discount);
            if($entry_id1){
...
//var_dump($grp_sttgs);exit;
                                                   ...
                                                   // echo $this->db->last_query();exit;
                                                    if($exe_qry && $exe_qry->num_rows()>0)
                                                    {
                                                        ...
                                                        if($exe_desig_id == $desig_type)
                                                        {
                                                                ...
                                                                $res11=$this->get_cm_type($lg_id,$cplg_id);
                                                                // var_dump($res11);echo $llogin_id;exit();
                                                                if($res11){
                                                                    //$flag =true;
                                                                    break;
                                                                }else{
                                                        ...
                                                        a: if($exe_desig_id == $desig_type)
                                                        {
                                                            ...
                                                            /*if($stype=='club_member' && $related_to=='CHANNEL_PARTNER'){
                                                                $res11=$this->get_cm_type($lg_id,$cplg_id);
                                                                //var_dump($res11);exit();
                                                                if($res11){
                                                                    //$flag =true;
                                                        ...
                                                        {
                                                            $get_new_id = $this->get_parent_from_login($noty_id,$lg_id, $desig_type,$parent_reward_rs,$entry_id1);
                                                            if(!empty($get_new_id)){
    ...
    function get_parent_from_login($noty_id,$login_id, $desig_type_id,$parent_reward_rs,$entry_id1){      
                ...
                if($get_parent_id_qry1->num_rows() >0)
                {
                    ...
                    if($type=='executive')
                        ...
                        if($exe_qry && $exe_qry->num_rows()>0)
                        {
                            ...
                            $investor = isset($result_sales['investor_type_id'])?$result_sales['investor_type_id']:0;
                            if(isset($investor)&&($investor>0)){
                                $slug = 'team_leader';
                                $des_type= get_designation_by_slug($slug);
                            }else{
                            ...
                            if($des_type == $desig_id)
                            {
                                ...
                                return  $this->get_parents_login($noty_id,$parent_id, $desig_type_id,$parent_reward_rs,$entry_id1);
                            }
                        }else{
                            return array();
                        }
    ...
    function get_parents_login($noty_id,$parent_id, $desig_type_id,$parent_reward_rs,$entry_id1){
        ...
        if($lg_qry && $lg_qry->num_rows()>0)
        {
            $lg_result = $lg_qry->row_array();
            $lg_id = $lg_result['id'];
           return $this->get_parent_from_login($noty_id,$lg_id,$desig_type_id,$parent_reward_rs,$entry_id1);
        }else{
           return $this->get_parent_from_login($noty_id,$parent_id,$desig_type_id,$parent_reward_rs,$entry_id1);
        }
    }
    ...
    function update_profile($image_file1,$image_file2){
       $loginsession = $this->session->userdata('logged_in_cp');
