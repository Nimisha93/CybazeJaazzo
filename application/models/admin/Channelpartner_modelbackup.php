...
Class Channelpartner_model extends CI_Model{
    ...
    function get_cpWallet(){
        ...
        $query="SELECT bn.id,bn.login_id,nc.name,bn.wallet_total,bn.bill_total,bn.purchased_on as purchase_date, DATE_FORMAT(bn.purchased_on,'%d-%b-%Y')as purchsed_on,bn.channel_partner_id FROM gp_purchase_bill_notification bn LEFT JOIN gp_login_table log ON log.id=bn.login_id 
            LEFT JOIN gp_normal_customer nc ON log.user_id=nc.id WHERE log.type='normal_customer' and bn.channel_partner_id = '$id'";
     ...
     function get_catNsubCategory()
  {
   ...
   function get_cpscategory(){
    ...
    function get_cpwallet_value($user_id){
    ...
    */
    ...
    function get_cpcustomer_connid($user_id){
      
        $qry="select cus.name,cus.email,logg.id logid,cp.id
            from
            gp_purchase_bill_notification noti 

            left join gp_login_table logg on logg.id=noti.login_id

            left join gp_normal_customer cus on cus.id=logg.user_id
            left join gp_pl_channel_partner cp on noti.channel_partner_id = cp.id

             where noti.channel_partner_id='$user_id' and logg.is_del='0'
             ";
        $res=$this->db->query($qry);
        if($res->num_rows()>0){
            $data['cus']=$res->result_array();
