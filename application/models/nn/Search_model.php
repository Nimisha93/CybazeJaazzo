...
//            return $querys->result_array();
    ...
    function get_search_products($key){
                $qry="SELECT p.id,p.name,pi.pi_image as image,p.description,p.special_prize as cost FROM gp_product_details p
                ...
                OR p.special_prize LIKE '%$key%' OR p.model LIKE '%$key%') order by p.view_count desc";
                // $qry="select name from gp_product_details
