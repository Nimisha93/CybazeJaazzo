...
/*print_r($partner['partner']);exit();*/
 ...
 foreach($agent['agent'] as $key=>$type){
...
<td><?php echo $key+1;?></td>
    ...
    <td><?php echo $type['club'];?></td>
   
    
    <td class="text-right">
    <a type="button" href="<?php echo base_url();?>admin/executives/edit_club_agent/<?php echo $type['id'];?>" class="btn btn-simple btn-info btn-icon edit"><i class="material-icons">edit</i></a>
...
<script type="text/javascript">
        ...
        $('.chck_grp_item').each(function () {
            ...
            rCallback: function(){
                $('.body_blur').show();
                        $.post('<?php echo base_url();?>admin/executives/delete_club_agent',{itemgrps:itemgrps}, function(data){
                            $('.body_blur').hide();
                            if(data.status){
            ...
            <script type="text/javascript">
                $(document).ready(function(){
                    ...
                    function load_demo(index) {
                        ...
                        $.post(base_url + "view_club_agent/" + index, { ajax: true,search:search}, function(data) {
                            ...
                            if(data.status){
                                ...
                                for(var i = 0; i< data2.length; i++){
                                    ...
                                    '<input type="hidden" name="id" class="id" value="'+data2[i].id+'">'+
                                        ...
                                        '<td>'+data2[i].club+'</td>'+
                                        '<td><a type="button" href="<?php echo base_url();?>admin/executives/edit_club_agent/'+data2[i].id+'" class="btn btn-simple btn-info btn-icon edit"><i class="material-icons">edit</i></a><input type="checkbox" name="" value="'+data2[i].id+'" class="chck_grp_item"></td>'+
