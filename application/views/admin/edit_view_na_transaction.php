...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function load_demo(index) {
            ...
            $.post(base_url + "approve_cp_transaction/" + index, { ajax: true,search:search}, function(data) {
                  ...
                  console.log(data);
                    ...
                    for(var i = 0; i< data1.length; i++){
                        ...
                        tr += '<tr>'+
                            ...
                            '<td class="total_amount">'+data1[i].amount+'</td>'+
                            '<td>'+detail+'</td>'+
                            '<td class="text-right"><a class="btn btn-primary approve_cp" data-toggle="tooltip" title="Approve"><i class="fa fa-check" aria-hidden="true"></i></a>'+
                            '<a class="btn btn-primary reject_cp" data-toggle="tooltip" title="Reject"><i class="fa fa-ban" aria-hidden="true" ></i></a></td>'+
                            '</tr>';
    ...
    ///////reject cp
   $(document).on('click','.reject_cp',function(){
    var cur=$(this);
    id = cur.parent().parent().find('.hiddentype_id').val();
        $('body').alertBox({
            title: 'Are You Sure?',
            ...
            rCallback: function(){
                $('.body_blur').show();
                $.post('<?php echo base_url();?>admin/home/reject_cp_transaction/',{id:id}, function(data){
                    $('.body_blur').hide();
                    if(data.status){
                        var center = '<div id="notifications-full"><div id="notifications-full-close" class="close"><span class="iconb" data-icon="&#xe20e;"></span></div><div id="notifications-full-icon"><span class="icon-thumbs-up" data-icon="&#xe261;"></span></div><div id="notifications-full-text">Successfully approved transaction</div></div>';
                        var effect='zoomIn';
