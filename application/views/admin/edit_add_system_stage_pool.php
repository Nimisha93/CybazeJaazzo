        ...
        $("#no_of_levels").keyup(function()
        {
            ...
            for(var i=0; i<level; i++)
            {
                ...
                new_row+='<div class="row_persantage"><div class="col-md-6 col-sm-12 col-xs-12 form-group"><label>Select Pool Stage </label>'+
                        ...
                        '<label>Allowed Percentage</label>'+
                        '<input class="form-control validate[required] allow" rows="3" name="design_allwed_persantage[]" placeholder="Allowed Persantage">'+
...
//            alert(level);
        ...
        $("#add_pool").click(function(e)
        {
            ...
            $.post('<?= base_url(); ?>/admin/pooling/add_new_stage_pool_data',data,function(data)
            {
                if(data.status)
                {
                    noty({text:"Successfully added new pool stage",type: 'success',layout: 'top', timeout: 3000});
