                    ...
                    <div class="clearfix"></div>
                        <div id="report_left_inner" class="report_left_inner" style="display: none">
                            <table id="purchase_table" class="table table-bordered table-striped" width="100%">
                                <thead>
                                    <tr class="filters">
                                        <th colspan="6">JAAZZO<br><p>Club Type Report</p></th>
                                    </tr>
                                    <tr class="tablbg">
                                        <th style="width: 45px">No</th>
                                        <th>Name</th>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function load_demo(index) {
            ...
            $.post(base_url + "clubtype_report/" + index, { ajax: true,search:search}, function(data) {
                $('tbody').html("");
                $('.body_blur').hide();
                if(data.status){
    ...
    $(document).on('click','.btn_print',function(){
        var base_url = "<?php echo base_url(); ?>";
        var search = $('#search').val();
        $.post(base_url + "admin/Report/print_club_type_report", {search:search}, function(data) {
            if(data.status){
