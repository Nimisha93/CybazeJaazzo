...
<div class="right_col" role="main">
 ...
 <?php      if (has_priv('add_clubmember'))
                                         { ?>
                        ...
                        <div class="">
                            <div class="row">
                                <div class="col-sm-offset-1 col-sm-3">
                                    <label class="pull-right">Club Package:</label>
                                </div>
                                <div class="col-sm-3">
                                    <select class="form-control typ" id="typ" name="typ">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Add Club Member</h4>
            </div>
            <form name="cm_forms" method="post" id="cm_forms" enctype="multipart/form-data"  action="<?php echo base_url(); ?>new_club_member">
            <div class="modal-body">
                ...
                <br>
                <div class="row plan" style="display: none;">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
            ...
            $('.body_blur').show();
                ...
                $.post('<?php echo base_url();?>get_clubplans_by_type',new_obj, function(data){
                    ...
                    if(data.status){
                        ...
                        var center = '<div id="notifications-full"><div id="notifications-full-close" class="close"><span class="iconb" data-icon="&#xe20e;"></span></div><div id="notifications-full-icon"><span class="icon-thumbs-down" data-icon="&#xe261;"></span></div><div id="notifications-full-text">Please try to select type</div></div>';
        ...
        function load_demo(index) {
            ...
            $.post(base_url + "all_club_members/" + index, { ajax: true,search:search,type:type}, function(data) {
                console.log(data);
                $('tbody').html("");
                $('.body_blur').hide();
                if(data.status){
                    ...
                    for(var i = 0; i< data1.length; i++){
                        ...
                        tr += '<tr>'+
                            '<td>'+sl_no+'</td>'+
                            '<td>'+data1[i].name+'</td>'+
                            '<td>'+ucwords(data1[i].un,true)+'<br>'+ucwords(data1[i].fix,true)+'</td>'+
                            '<td>'+data1[i].phone+'</td>'+
                            '<td>'+data1[i].email+'</td>'+
                            '<td><a href="'+base_url+"admin/Clubmember/get_member_byid/"+data1[i].user_id+'"><button type="button" class="btn btn-primary type_sub"><i class="fa fa-pencil"></i> </button></a><input type="checkbox" name="" value="'+data1[i].user_id+'" class="chck_item_id"></td>'+
                            '</tr>';
    ...
    $(document).ready(function() {
        ...
        var datas = { 
            ...
            success:   function(data){
              $('.body_blur').hide();
              if(data.status){
                var center = '<div id="notifications-full"><div id="notifications-full-close" class="close"><span class="iconb" data-icon="&#xe20e;"></span></div><div id="notifications-full-icon"><span class="icon-thumbs-up" data-icon="&#xe261;"></span></div><div id="notifications-full-text">Club Member Added Successfully </div></div>';
                var effect='zoomIn';
                $("#notifications").append(center);
                $("#notifications-full").addClass('animated ' + effect);
                refresh_close();
