                    ...
                    <div class="x_content">
                        <div class="">
                            <div class="table-responsive tabmargntp30">
                                <form method="post" name="mail_form" id="mail_form" enctype="multipart/form-data">
                                    <div class="col-md-12">

                                        <div class="col-md-12 col-sm-6 col-xs-12 form-group">
                                            <label>Cusomers</label>
                                            <select id="customer" class="form-control validate[required] search-box-open-up search-box-sel-all" name="customer[]" multiple="multiple" onchange="console.log($(this).val())">
                                                <?php foreach($customers['cus'] as $cus){?>
                                                <option value="<?php echo $cus['email'];?>"><?php echo $cus['name'];?>(<?php echo $cus['email'];?>)</option>
                                                <?php } ?>
                                            </select>
                                            <?php foreach($customers['cus'] as $cus){?>
                                            <input type="hidden" name="connctid[]" value="<?php echo $cus['conid'];?>">
                                            <?php } ?>
                                        </div>
                                        <div class="col-md-12 col-sm-12 col-xs-12 form-group">
                                            <label>Message</label>
                                            <textarea class="form-control" title="Message" name="notification" rows="3" placeholder="Message" class="form-control validate[required]"></textarea>
                                        </div>

                                        <div class="col-md-12 col-sm-12 col-xs-12 form-group">
                                            <input type="submit" class="btn btn-primary" name="mailsub" id="mailsub" value="Send">
                                        </div>
                                    </div>
                                </form>
                            </div>
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                $('#mailsub').click(function(e){
                    e.preventDefault();
                    var sta = $("#mail_form").validationEngine("validate");
                    if(sta== true){
                        ...
                        $.post('<?php echo base_url();?>admin/Channel_partner/cp_send_notification', data, function(data){
                            $('.body_blur').hide();

                            if(data.status){
                                noty({text:"Successfully send",type: 'success',layout: 'top', timeout: 3000});
                                $('#mail_form')[0].reset();
                            }
                            else{
